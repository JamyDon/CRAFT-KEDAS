...
from ..evaluate import compute_edit_quality, compute_edit_quality_for_case, compute_icl_edit_quality, compute_sent_metric
...
class BaseEditor:
    ...
    def edit_for_case(self,
        ...
        return self.edit_requests_for_case(requests=requests, verbose=verbose, test_generation=test_generation, **kwargs)
    ...
    def edit_requests(self,
        ...
        if 'pre_edit' in kwargs and kwargs['pre_edit'] is not None:
            ...
            for i, request in enumerate(tqdm(requests)):
                if self.alg_name == 'IKE-easyedit':
                    assert 'train_ds' in kwargs.keys(), print('IKE-easyedit need train_ds(For getting In-Context prompt)')
        ...
        def edit_func(request: Union[Dict, List[Dict]], keep_original_weight=False):
            if self.alg_name == 'IKE-easyedit' or self.alg_name == 'ICE':
                edited_model, weights_copy, icl_examples = self.model, {}, self.apply_algo(
                    ...
                    train_ds=kwargs['train_ds'] if self.alg_name == 'IKE-easyedit' else None
                )
            else:
                edited_model, weights_copy = self.apply_algo(
                    self.model,
    ...
    def edit_requests_for_case(self,
             requests,
             verbose=True,
             test_generation=False,
             **kwargs
        ...
        all_metrics = []
        for i, request in enumerate(tqdm(requests)):
            metrics = {"pre": compute_edit_quality_for_case(self.model, self.model_name, self.hparams, self.tok, request, self.hparams.device, eval_metric=eval_metric, test_generation=test_generation, pre_edit=True)}
            all_metrics.append(metrics)
        ...
        def edit_evaluation(all_metrics, request, edited_model, idx, test_generation, icl_examples, **kwargs):
            ...
            all_metrics[idx].update({
                'case_id': idx,
                "requested_rewrite": request,
                "post": compute_edit_quality_for_case(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, eval_metric=eval_metric, test_generation=test_generation),
            })
