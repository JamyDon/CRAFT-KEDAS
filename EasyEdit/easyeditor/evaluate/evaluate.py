...
def compute_edit_quality(
    ...
    # print()
    # print("=="*20)
    # print(f"Evaluating rewrite for {target_new} with ground truth {ground_truth}")
    # print()
    ...
    ret = compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                              rewrite_prompts, target_new, device=device, eval_metric=eval_metric, vanilla_generation=vanilla_generation, pre_edit=pre_edit)
    ...
    if rephrase_prompts is not None:
        ret.update(
            compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                                rephrase_prompts, target_new, device=device, test_rephrase=True, eval_metric=eval_metric, vanilla_generation=vanilla_generation, pre_edit=pre_edit)
        )

    # print()
    # print("=="*20)
    # print(f"Evaluating locality for {target_new} with ground truth {ground_truth}")
    # print()
            ...
            ret['locality'].update(
                compute_locality_quality(model, model_name, hparams, tok, locality_key,
                                         ...
                                         record['locality'][locality_key]['ground_truth'], device=device,
                                            vanilla_generation=vanilla_generation, pre_edit=pre_edit)
            )

    # print()
    # print("=="*20)
    # print(f"Evaluating portability for {target_new} with ground truth {ground_truth}")
    # print()
    ...
    if test_generation:
        if hparams.alg_name == 'GRACE' or vanilla_generation:
            ret['fluency'] = test_generation_quality(model=model,tok=tok,alg=hparams.alg_name,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=True, pre_edit=pre_edit)
        else:
            ret['fluency'] = test_generation_quality(model=model,tok=tok,alg=hparams.alg_name,prefixes=rewrite_prompts if isinstance(rewrite_prompts,list) else [rewrite_prompts,], max_out_len=100, vanilla_generation=False, pre_edit=pre_edit)
    return ret

def compute_edit_quality_craft(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    if isinstance(model,LORA):
        model=model.model
    # First, unpack rewrite evaluation record.
    target_news = record['target_new']

    rewrite_prompts = record["prompt"]
    rewrite_accs = []
    for rewrite_prompt, target_new in zip(rewrite_prompts, target_news):
        raw_ret = compute_rewrite_or_rephrase_quality(model, model_name, hparams, tok,
                                                rewrite_prompt, target_new, device=device, eval_metric=eval_metric, vanilla_generation=vanilla_generation, pre_edit=pre_edit)
        rewrite_accs.append(raw_ret['rewrite_acc'])
    rewrite_acc = np.mean(rewrite_accs)
    ret = {
    ...
    if 'locality' in record.keys() and any(record['locality']):      
        for locality_key, items in record['locality'].items():            
            for i, item in enumerate(items):
                ret['locality'].update(
                    compute_locality_quality(model, model_name, hparams, tok, 
                                        locality_key,
                                        item['prompt'],
                                        item['ground_truth'][0], 
                                        device=device,
    ...
    if 'portability' in record.keys() and any(record['portability']):
        for portability_key, items in record['portability'].items():
            for i, item in enumerate(items):
                ret['portability'].update(
                    compute_portability_quality(model, model_name, hparams, tok, 
                                            f"{portability_key}_{i}",  # 确保key唯一
                                            item['prompt'],
                                            item['ground_truth'][0], 
                                            device=device,
                                            vanilla_generation=vanilla_generation, 
...
def compute_edit_quality_for_case(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    rewrite_prompts = record["prompt"]
    rephrase_prompts = record["rephrase_prompt"] if 'rephrase_prompt' in record.keys() else None
    ret = compute_rewrite_or_rephrase_quality_for_case(model, model_name, hparams, tok,
                                              rewrite_prompts, target_new, device=device, eval_metric=eval_metric, vanilla_generation=vanilla_generation, pre_edit=pre_edit)
    ...
    if rephrase_prompts is not None:
        ret.update(
            compute_rewrite_or_rephrase_quality_for_case(model, model_name, hparams, tok,
                                                rephrase_prompts, target_new, device=device, test_rephrase=True, eval_metric=eval_metric, vanilla_generation=vanilla_generation, pre_edit=pre_edit)
        )
    ...
    if 'locality' in record.keys() and any(record['locality']):
        for locality_key in record['locality'].keys():
            ret['locality'].update(
                compute_locality_quality_for_case(model, model_name, hparams, tok, locality_key,
                                         record['locality'][locality_key]['prompt'],
                                         record['locality'][locality_key]['ground_truth'], device=device,
                                            vanilla_generation=vanilla_generation, pre_edit=pre_edit)
            )
    ...
    if 'portability' in record.keys() and any(record['portability']):
        for portability_key in record['portability'].keys():
            ret['portability'].update(
                compute_portability_quality_for_case(model, model_name, hparams, tok, portability_key,
                                            record['portability'][portability_key]['prompt'],
                                            record['portability'][portability_key]['ground_truth'], device=device,
                                            vanilla_generation=vanilla_generation, pre_edit=pre_edit)
            )
...
def compute_rewrite_or_rephrase_quality(
    ...
    if hasattr(hparams, 'evaluation_type') and hparams.evaluation_type == "LLM-judge":
        ...
        if eval_metric == 'ppl':
        ...
        elif hparams.alg_name=="GRACE" or vanilla_generation:
            # ppl = PPL(model, tok, prompt, target_new, device)
            if 't5' in model_name.lower():
                acc = test_seq2seq_batch_prediction_acc(model, tok, hparams, prompt, target_new, device)
            else:
                acc = test_prediction_acc(model, tok, hparams, prompt, target_new, device, vanilla_generation=True, pre_edit=pre_edit)
            ...
            if 't5' in model_name.lower():
                ...
                acc = test_prediction_acc(model, tok, hparams, prompt, target_new, device, pre_edit=pre_edit)
            ret = {
                f"{key}_acc": acc
            }
    return ret

def compute_rewrite_or_rephrase_quality_for_case(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    if not test_rephrase:
        ...
        key = 'rephrase'
    accs, gen_contents = test_prediction_acc_for_case(model, tok, hparams, prompt, target_new, device, vanilla_generation=True, pre_edit=pre_edit)
    # ret = {
    #     f"{key}_acc": acc,
    #     f"{key}_gen_content": gen_content
    # }     
    ret = {}
    idx = 0
    for acc, gen_content in zip(accs, gen_contents):
        ret[f"{key}_acc_{idx}"] = acc
        ret[f"{key}_gen_content_{idx}"] = gen_content[idx]
        idx += 1
    ret[f"{key}_acc"] = accs
    return ret
...
def compute_locality_quality(
    ...
    if hasattr(hparams, 'evaluation_type') and hparams.evaluation_type == "LLM-judge":
        ...
        if 't5' in model_name.lower():
            ...
            loc_tokens = test_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device, locality=True, vanilla_generation=(hparams.alg_name=='GRACE' or vanilla_generation), pre_edit=pre_edit)
...
def compute_locality_quality_for_case(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    loc_accs, gen_contents = test_prediction_acc_for_case(model, tok, hparams, prompt, locality_ground_truth, device, locality=True, vanilla_generation=(hparams.alg_name=='GRACE' or vanilla_generation), pre_edit=pre_edit)
    # if type(loc_accs) is not list:
    #     loc_accs = [loc_accs,]
    ret = {}
    idx = 0
    for acc, gen_content in zip(loc_accs, gen_contents):
        ret[f"{locality_key}_output_{idx}"] = acc
        ret[f"{locality_key}_gen_content_{idx}"] = gen_content
        idx += 1
    ret[f"{locality_key}_output"] = loc_accs
    return ret
...
def compute_portability_quality(
    ...
    if hasattr(hparams, 'evaluation_type') and hparams.evaluation_type == "LLM-judge":
        ...
        if 't5' in model_name.lower():
            ...
            portability_correct = test_prediction_acc(model, tok, hparams, prompt, ground_truth, device, vanilla_generation=(hparams.alg_name=='GRACE' or vanilla_generation), pre_edit=pre_edit)
...
def compute_portability_quality_for_case(
    model,
    model_name,
    hparams: HyperParams,
    tok: AutoTokenizer,
    ...
    portability_accs, gen_contents = test_prediction_acc_for_case(model, tok, hparams, prompt, ground_truth, device, vanilla_generation=(hparams.alg_name=='GRACE' or vanilla_generation), pre_edit=pre_edit)

    ret = {}
    idx = 0
    for acc, gen_content in zip(portability_accs, gen_contents):
        ret[f"{portability_key}_acc_{idx}"] = acc
        ret[f"{portability_key}_gen_content_{idx}"] = gen_content
        idx += 1
    ret[f"{portability_key}_acc"] = portability_accs
    return ret  
