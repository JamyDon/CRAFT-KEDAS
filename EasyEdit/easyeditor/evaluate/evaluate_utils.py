...
import nltk
import typing
from ..util.generate import generate_fast, generate_kedas
...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False, pre_edit=False):
    if vanilla_generation:
        if isinstance(prompts, str):
            prompts, targets = [prompts, ], [targets, ]
        results = []
        for prompt, target_new in zip(prompts, targets):
            if hparams.alg_name == 'KEDAS':
                prefix = model.retrieve_prefix(prompt)
                lora = False
                ...
                if (hparams.alg_name == 'LTE' or hparams.alg_name == 'EREN' or hparams.alg_name == 'IKE') and not pre_edit:
                    prefix = model.retrieve_prefix(prompt)
                    prompt = prefix + prompt
                target_new_tokens = tok.encode(target_new, add_special_tokens=False)
                prompt_tok = tok(
                ...
                print(f"Evaluating pre_edit: {pre_edit} for alg: {hparams.alg_name}")
                if hparams.alg_name == 'LTE':
                    gen_token = model.generate(
                        pre_edit=pre_edit,
                        input_ids=prompt_tok['input_ids'],
                        attention_mask=prompt_tok['attention_mask'],
            ...
            gen_token = gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens):]
            if locality:
                # results.append(gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens):])
                if len(gen_token) < len(target_new_tokens):
                    gen_token = gen_token + [tok.pad_token_id] * (len(target_new_tokens) - len(gen_token))
                results.append(np.mean(np.equal(target_new_tokens, gen_token)))
            else:
                # gen_token = gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens):]
                if len(gen_token) < len(target_new_tokens):
                    gen_token = gen_token + [tok.pad_token_id] * (len(target_new_tokens) - len(gen_token))
    ...
    if hparams.alg_name == 'KEDAS':
        for i in range(len(prompts)):
            prefix = model.retrieve_prefix(prompts[i])
            if prefix is not None:
                prompts[i] = prefix + prompts[i]
                loras.append(True)
            else:
                loras.append(False)
    elif (hparams.alg_name == 'LTE' or hparams.alg_name == 'EREN' or hparams.alg_name == 'IKE') and not pre_edit:
        for i in range(len(prompts)):
            prefix = model.retrieve_prefix(prompts[i])
    ...
    with torch.no_grad():
        if hparams.alg_name == 'KEDAS':
            ...
            for i in range(len(prompts)):
                outputs = model(
                    prompt_target_tok['input_ids'][i].unsqueeze(0).to(f"cuda:{device}"),
                    attention_mask=prompt_target_tok['attention_mask'][i].unsqueeze(0).to(f"cuda:{device}"),
                    lora=loras[i],
                )
                logits = outputs.logits
                answer = torch.argmax(logits, dim=-1).squeeze().detach().cpu().numpy().tolist()
                answer = slice_list(answer,[prompt_len[i]], left=True)
            ...
            if prompt_target_tok['input_ids'].shape[0] >= 16:
                answers = []
                for i in range(len(prompts)):
                    input_ids = prompt_target_tok['input_ids'][i].unsqueeze(0).to(f"cuda:{device}")
                    attention_mask = prompt_target_tok['attention_mask'][i].unsqueeze(0).to(f"cuda:{device}")
                    model_input = {
                        'input_ids': input_ids,
            ...
            if isinstance(answers[0], list):
                ...
                for ans,label in zip(answers,labels):
                    if len(ans) > len(label):
                        ...
                        ans = [tok.pad_token_id] * (len(label) - len(ans)) + ans
                    assert len(ans) == len(label)
                    res.append(ans)
            else:
                if len(answers) > len(labels):
                    answers = answers[-len(labels):]
                elif len(answers) < len(labels):
                    answers = [tok.pad_token_id] * (len(labels) - len(answers)) + answers
                res = answers
            return res if type(res[0]) is list else [res,]
            # return answers if type(answers[0]) is list else [answers,]
...
def test_prediction_acc_for_case(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False, pre_edit=False):
    if isinstance(prompts, str):
        prompts, targets = [prompts, ], [targets, ]
    results = []
    original_gen_tokens = []
    ...
    for prompt, target_new in zip(prompts, targets):
        ...
        decoded_gen_token = tok.decode(gen_token[0][-len(target_new_tokens):])
        gen_token = gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens):]
        if locality:
            if len(gen_token) < len(target_new_tokens):
                gen_token = gen_token + [tok.pad_token_id] * (len(target_new_tokens) - len(gen_token))
            ...
            if len(gen_token) < len(target_new_tokens):
                gen_token = gen_token + [tok.pad_token_id] * (len(target_new_tokens) - len(gen_token))
            results.append(np.mean(np.equal(target_new_tokens, gen_token)))
        original_gen_tokens.append(decoded_gen_token)
        gen_contents.append(gen_token)
    if locality:
        return gen_contents, original_gen_tokens
    return results, original_gen_tokens
...
def test_generation_quality(
    ...
    pre_edit: Optional[bool] = False,
):
    if alg == "KEDAS":
        gen_texts = []
        for prefix in prefixes:
            gen_text = generate_kedas(
                model,
                tok,
                prefix,
                n_gen_per_prompt=1,
...
def F1(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=True):
    if vanilla_generation:
        ...
        if len(gen_token) < len(target_new_tokens):
            gen_token = gen_token + [tok.pad_token_id] * (len(target_new_tokens) - len(gen_token))
        return f1_score(target_new_tokens, gen_token, average='macro')
