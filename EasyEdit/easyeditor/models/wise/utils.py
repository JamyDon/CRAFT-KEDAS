...
def tokenize(batch, tokenizer, device, context_templates=None, hparams=None):
    ...
    loc_prompts = []
    for item in batch:
        if 'locality' in item and len(item['locality']) > 0 and 'prompt' in item['locality'][list(item['locality'].keys())[0]] and len(item['locality'][list(item['locality'].keys())[0]]['prompt']) > 0:
            loc_prompts.append(item['locality'][list(item['locality'].keys())[0]]['prompt'][0])
        else:
            loc_prompts.append('')
    # loc_prompts = [item['locality'][item['locality'].keys()[0]]['prompt'][0] for item in batch]
