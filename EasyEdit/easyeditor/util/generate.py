...
def generate_fast(
    ...
    if vanilla_generation:
        ...
        gen_txt = gen_txt.detach().cpu().numpy().tolist()
        txt = [tok.decode(x, skip_special_tokens=True) for x in gen_txt]
        txt = [
    ...
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            ...
            past_key_values = model_out.past_key_values if hasattr(model_out, 'past_key_values') else None
...
def generate_kedas(
    ...
    # Unroll prompts and tokenize
    inp = [prompt for prompt in [prompt] for _ in range(n_gen_per_prompt)]

    lora = False
    prefix = model.retrieve_prefix(prompt)
    if prefix is not None:
        inp = [prefix + prompt for prompt in inp]
        lora = True
